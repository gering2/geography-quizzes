...
export default function QuizAnswer({setNumberCorrect,setNumberGuessed,numberCorrect,numberGuessed,answer,country,setQuestionAnswered, questionAnswered}) {
  ...
  return (
    <button disabled={questionAnswered} onClick = {(e) => handleAnswerClick(e)} class={`mb-2 mt-1 flex justify-center text-white text-opacity-90  w-[40%] py-6 text-2xl rounded-md ${incorrectAnswer?"bg-[#D10203]":correctAnswer?"bg-[#00FF00]":"bg-[#333333]"}`}>{answer}</button>
  )
}
