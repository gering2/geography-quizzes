...
export default function QuizAnswers(props) {
  ...
  return (
    <div class=" flex flex-col h-full items-center">
      ...
      {!quizFinished? <button disabled = {!questionAnswered} onClick = {props.chooseRandomCountry} class="rounded-sm mb-4 px-3 py-2 mt-6 bg-red-700  hover:bg-red-900 text-white cursor-pointer">Next&nbsp;{/*String.fromCharCode(8594)*/}</button>: <button  disabled={!questionAnswered} onClick = {props.setShowModal} class="px-3 py-2 mt-2 bg-red-700  hover:bg-red-900 text-white cursor-pointer">Next&nbsp;{/*String.fromCharCode(8594)*/}</button>}
    </div>
  )
}
